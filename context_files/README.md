...
| --- | --- | --- |
| [`step-26-as-built.md`](./step-26-as-built.md) | **Start here.** h2o validated — **15/15 on parquet and 15/15 on CSV**, the latter being the only end-to-end check step 22's CSV rewrite can get. The run also supplied the measurement steps 22–24 never had: **the suite is 2.31x faster than the published numbers (6.42x → 2.78x vs polars), and gb8 — the worst query in the suite — is 4.95x faster**, which is step 24's radix prediction cashing out. Read it also for the reason the machine could not run it at first: 38 leaked `ursus-micro-*` fixtures, 6.5 GB, on a RAM-backed `/tmp`, from my own benchmark runs — fixed with `TestMain`, which is what `b.TempDir()` cannot be for a process-scoped fixture. Plus a 40% peak-memory regression on gb10. | Always. Authoritative over everything below. |
